    ...
    const handleSelectAll = () => {
        setSelectedIds((current) => 
            current.length === filteredAndSortedTransactions.length
                ? []
                : filteredAndSortedTransactions.map((t: { id: string }) => t.id)
        );
    };

    const handleBulkDelete = async () => {
        if(!window.confirm(`Are you sure you want to delete ${selectedIds.length} transactions? This action cannot be undone.`)) return;
            ...
            <Table>
                <TableHeader>
                    <TableRow>
                        ...
                        <TableHead>Descrition</TableHead>
                ...
                <TableBody>
                    ...
                    filteredAndSortedTransactions.map((transaction: Transaction) => (
                        ...
                        <TableCell className="capitalize">
                            <span style={{backgroundColor: (categoryColors as Record<string, string>)[transaction.category]}} className="px-2 py-1 rounded text-white text-sm" >
                                ...
                                <Tooltip>
                                    <TooltipTrigger>
                                        <Badge variant="outline" className="gap-1 bg-purple-100 text-purple-700 hover:bg-purple-200">
                                            ...
                                            {transaction.recurringInterval ? RECURRING_INTERVALS[transaction.recurringInterval] : 'Monthly'}
                                        </Badge>
                                    </TooltipTrigger>
                                    <TooltipContent>
                                        <div className="text-sm">
                                            ...
                                            <div>
                                                {transaction.nextRecurringDate ? format(new Date(transaction.nextRecurringDate), "PP") : "N/A"}
                                            </div>
                                        </div>
                                    </TooltipContent>
                                </Tooltip>
