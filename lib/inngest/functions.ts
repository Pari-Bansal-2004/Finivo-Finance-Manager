...
export const triggerRecurringTransactions = inngest.createFunction({
  ...
  if(recurringTransactions.length>0){
    const events = recurringTransactions.map((transaction: { id: string; userId: string })=>({
...
export const processRecurringTransactions = inngest.createFunction({
  ...
  async({event,step})=>{
    const { transactionId, userId } = event.data as { transactionId?: string; userId?: string };
...
function isTransactionDue(transaction: { lastProcessed: Date | null; nextRecurringDate: Date }) {
...
function calculateNextRecurringDate(startDate: Date | string, interval: string) {
...
async function generateFinancialInsights(stats: { totalExpenses: number; totalIncome: number; byCategory: Record<string, number>; transactionCount: number }, month: string){
...
const getMonthlyStats = async(userId: string, month: Date) => {
  ...
  return transactions.reduce(
    (stats: { totalExpenses: number; totalIncome: number; byCategory: Record<string, number>; transactionCount: number }, t: any)=>{
