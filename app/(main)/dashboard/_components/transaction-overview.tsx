...
const DashboardOverview = ({ accounts, transactions }: { accounts: Account[]; transactions: Transaction[] }) => {
    ...
    const expensesByCategory = currentMonthExpenses.reduce((acc,transaction)=>{
        ...
        if(!(acc as Record<string, number>)[category]){
            (acc as Record<string, number>)[category] = 0;
        }
        (acc as Record<string, number>)[category] += transaction.amount;
        return acc;
    },{});
