...
const AccountCard = ({ account }: { account: { name: string; type: string; balance: number; id: string; isDefault: boolean } }) => {
...
useEffect(() => {
    if(error){
        toast.error((error as any)?.message || "Failed to update default account");
    }
}, [error]);
